...
/* While Mongo is a schema-less database, meaning we can
   just store arbitrary objects in it, Mongoose
   ...
   The schema enforces a semi-rigid format on our
   data so that it is more of a "known quantity". This
   ...
   objects, the type usually does not matter. However, this
   is something added by Mongoose to clarify what each
   variable should contain. We also give it the required
   option, which means every object in the cat database MUST
   have a name. We set the trim option to true, which will
